  ...
  <v-container>
    <v-row class="row">
      ...
      <v-col>
        <v-list>
<v-list-item-group>
  <v-list-item v-for='people in people' :key="people">
    ...
    <!-- <div class="hello">
      ...
      <div>
        <ul>
          <li v-for="people in people" :key="people">{{ people.name }}</li>
        </ul>
      </div>
...
export default {
  ...
  mounted() {
    swapiModule.getPeople({ }, (data) => {
      ...
      return this.people = data.results, this.nextPage = data.next, this.previousPage=data.previous
    });
  },
  methods: {
    getName() {
